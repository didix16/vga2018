...
Eximo.loadSpriteSheet = function (url, urlPrefix, callback) {
	$.getJSON(url, function(data) {
		urlPrefix = typeof urlPrefix === "string" ? urlPrefix : "";

		let pos = url.lastIndexOf("/");
		let file = url.substring(pos+1);
		file = file.split(".")[0];


		Eximo.loadSheet(urlPrefix+data.meta.image, function (img) {
			for (var spriteName in data.frames) {
				Eximo.sprites[file+spriteName] = data.frames[spriteName];
				Eximo.sprites[file+spriteName].sheet = img;
			}
